          ...
          {faqs.map((faq, index) => {
            ...
            return (
              <motion.div 
                ...
                className={`rounded-2xl border transition-all duration-300 ${isOpen ? 'bg-slate-900 border-[#4F8CFF] shadow-lg shadow-blue-500/10' : 'bg-slate-900/40 border-white/5 hover:border-white/15'}`}
              >
                <button
                  onClick={() => setOpenIndex(isOpen ? null : index)}
                  className="w-full flex items-center justify-between p-6 md:p-8 text-left focus:outline-none cursor-pointer group"
                >
                  <span className={`text-lg font-bold transition-colors ${isOpen ? 'text-[#4F8CFF]' : 'text-white'}`}>
                    {faq.question}
                  </span>
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 transition-colors ${isOpen ? 'bg-[#4F8CFF] text-white' : 'bg-white/5 text-slate-400 group-hover:bg-white/10'}`}>
